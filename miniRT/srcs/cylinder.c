...
/*   Updated: 2022/05/25 11:02:08 by sangjeon         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "minirt.h"

int	set_cy_quadratic(t_ray *ray, t_cylinder *cy, t_quadratic *quadratic)
{
	t_vec3	vec_o;
	t_vec3	vec_d;
	t_vec3	vec_e;
	...
	quadratic->c = vec3_len_squared(vec_o) + \
	vec3_len_squared(vec_e) - 2 * vec3_dot(vec_o, vec_e) - cy->r * cy->r;
	return (1);
}
...
int	cy_intersect(void *shape, t_ray *ray, float *t)
{
	...
	cy = shape;
	if (set_cy_quadratic(ray, cy, &quadratic) == 0)
		return (0);
	if (!sol_quadratic(&quadratic))
		return (0);
	if (quadratic.x0 > quadratic.x1)
		swap_float(&(quadratic.x0), &(quadratic.x1));
	if (quadratic.x0 < 0)
	{
	...
	quadratic.tmp = vec3_dot(vec3_minus(ray_at(quadratic.x0, *ray), \
	cy->origin), cy->n);
