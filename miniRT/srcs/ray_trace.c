...
/*   Updated: 2022/05/25 10:34:42 by sangjeon         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	*pixel = get_vec3_same_val(0.0f);
	if (trace(objs, &hit_obj, ray, &t_near))
	{
		...
		while (lights)
		{
			...
			if (light->id == L)
				*pixel = vec3_plus(*pixel, \
				light_color(objs, hit_obj, &surface, light));
			lights = lights->next;
		}
	}
	adjust_rgb(pixel);
...
void	ray_trace(t_image *image, t_camera *cam, t_list *objs, t_list *lights)
{
	...
	while (y < image->height)
	{
		...
		while (x < image->width)
		{
			camera_ray = get_camera_ray(x, y, image, cam);
			trace_and_setcolor(&camera_ray, objs, \
			lights, &(image->pixel[y][x]));
			x++;
		}
		y++;
	}
