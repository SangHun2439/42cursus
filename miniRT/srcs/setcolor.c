...
/*   Updated: 2022/05/25 10:24:36 by sangjeon         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	return (vec3_multi(hit_obj->rgb, \
	vec3_multi_scalar(light->bright, light->rgb)));
}
...
int	is_shadow(t_list *objs, t_surface *surface, t_light *light)
{
	...
	light_ray.origin = vec3_plus(surface->p_hit, \
	vec3_multi_scalar(0.5, surface->n_hit));
	t_max = vec3_len(vec3_minus(light->origin, light_ray.origin));
	light_ray.direction = vec3_unit(vec3_minus(light->origin, \
	light_ray.origin));
	t = INFINITY;
	while (objs)
	{
...
t_vec3	diffuse_color(t_objs *hit_obj, t_surface *surface, t_light *light)
{
	...
	if (intensity < 0)
		intensity = 0;
	return (vec3_multi(hit_obj->rgb, vec3_multi_scalar(intensity * \
	light->bright / (vec3_len_squared(p_to_l) * DISTANCE_CONST), light->rgb)));
}
...
t_vec3	specular_color(t_surface *surface, t_light *light)
{
	...
	return (vec3_multi_scalar(specular * light->bright \
	/ (vec3_len_squared(p_to_l) * DISTANCE_CONST), light->rgb));
}
...
t_surface *surface, t_light *light)
{
	...
	if (is_shadow(objs, surface, light))
		return (rgb);
	rgb = vec3_plus(rgb, vec3_multi_scalar(DIFFUSE_CONST, \
	diffuse_color(hit_obj, surface, light)));
	rgb = vec3_plus(rgb, vec3_multi_scalar(SPECULAR_CONST, \
	specular_color(surface, light)));
	return (rgb);
}
